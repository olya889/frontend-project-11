...
export default (elements, i18n, state) => {
  const { form, feedbackElement, postsContainer, feedsContainer, inputElement } = elements;
...
/* export const renderError = (feedbackElement, inputElement, watchedState, i18nInstance) => {
  feedbackElement.textContent = i18nInstance.t(watchedState.error);
  feedbackElement.classList.remove('text-success');
  feedbackElement.classList.add('text-danger');
  inputElement.classList.add('is-invalid');
};

export const renderRSS = (form, postsContainer, feedsContainer, inputElement, watchedState, i18nInstance) => {
    postsContainer.replaceChildren();
    feedsContainer.replaceChildren();
    inputElement.classList.remove('is-invalid');
    const feedsBody = document.createElement('div');
...
export const renderPosts = (postsContainer, value, watchedState, i18nInstance) => {
  const postsBody = document.createElement('div');
      postsBody.classList.add('card', 'border-0');
      const postsTitleDiv = document.createElement('div');
      const postsTitle = document.createElement('h2');
